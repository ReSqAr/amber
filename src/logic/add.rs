use crate::db::models::{
    InsertBlob, InsertFile, InsertFileBlobMaterialisation, InsertMaterialisation,
};
...
pub(crate) async fn add_files(
    ...
    let (db_tx, db_rx) = flightdeck::tracked::mpsc_channel(
        "add_files::db",
    ...
    let db_handle = {
        ...
        tokio::spawn(async move {
            local_repository
                .add_file_blob_materialisations(db_rx)
                .await
                .inspect_err(|e| {
                    log::error!("add_files: add_file_blob_materialisations task failed: {e}")
                })
        })
    };
        ...
        let stream = tokio_stream::StreamExt::map(
            ...
            |file_result: Result<VirtualFile, InternalError>| {
                ...
                async move {
                    ...
                    let record = InsertFileBlobMaterialisation {
                        file,
                        blob,
                        materialisation: mat,
                    };
                    ...
                    db_tx
                        .send(record)
                        .await
                        .inspect_err(|e| log::error!("add_files: send on db_tx failed: {e}"))?;
