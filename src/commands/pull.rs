...
use crate::transport::server::invariable::{DownloadRequest, RepositoryIdRequest, RepositoryIdResponse};
...
pub async fn pull(port: u16) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let CurrentRepository { repo_id: local_repo_id } = db.get_or_create_current_repository().await?;
    ...
    let RepositoryIdResponse { repo_id: remote_repo_id } = client.repository_id(repo_id_request).await?.into_inner();
    info!("remote repo_id={}", remote_repo_id);

    let missing_blobs = db
        .missing_blobs(&remote_repo_id, &local_repo_id)
        .await?;

    for BlobObjectId { object_id } in missing_blobs.into_iter() {
        let content = client
            .download(DownloadRequest { object_id: object_id.clone() })
            .await?
            .into_inner()
            .content;
...
async fn reconcile_filesystem(db: DB, root: PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for FilePathWithObjectId { path: relative_path, object_id } in db.desired_filesystem_state(&repo_id).await? {
